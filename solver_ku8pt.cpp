...
std::size_t updateNumberofIters(double confidence, double error_prob, std::size_t n_points, std::size_t n_iters) {
    ...
    error_prob = std::max(error_prob, 0.);
    error_prob = std::min(error_prob, 1.);

    double num = std::max(1. - confidence, std::numeric_limits<double>::min());
    double denom = 1. - pow(1. - error_prob, n_points);
    ...
    return static_cast<size_t>(denom >= 0 || -num >= n_iters * (-denom) ?
