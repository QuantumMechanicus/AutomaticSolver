...
AutomaticEstimator::AutomaticEstimator(double w, double h, const AutomaticEstimator::Points &u1d,
    ...
    u1d_.row(0) = u1d.row(0) - ones * w_ / 2.0;

    u1d_.row(1) = u1d.row(1) - ones * h_ / 2.0;
...
AutomaticEstimator::PairOfMatricesFandLambdas
...
double AutomaticEstimator::estimate(Eigen::Matrix3d &F, double &Lambda, const std::string &name_f,
    ...
    for (std::size_t k = 0; k < numberOfIterations; ++k) {
        ...
        subsetQ1 << u1d_.col(numbers[0]), u1d_.col(numbers[1]), u1d_.col(numbers[2]), u1d_.col(
                numbers[3]), u1d_.col(numbers[4]), u1d_.col(numbers[5]), u1d_.col(numbers[6]), u1d_.col(numbers[7]);
    ...
    findInliers(Lambda, F, min_quantile, inliers_f);
    return min_quantile;
}
...
AutomaticEstimator::computeErrors(double hyp_lambda, const Eigen::Matrix3d &hyp_F, std::vector<double> &left_errors,
    ...
    r1d.resize(Eigen::NoChange, u1d_.cols());
    r1d.row(0) = (u1d_.row(0).cwiseProduct(u1d_.row(0)) + u1d_.row(1).cwiseProduct(u1d_.row(1)));
    r1d.row(1) = (u1d_.row(0).cwiseProduct(u1d_.row(0)) + u1d_.row(1).cwiseProduct(u1d_.row(1)));

    Eigen::Matrix<double, 2, Eigen::Dynamic> r2d;
    r2d.resize(Eigen::NoChange, u2d_.cols());

    r2d.row(0) = (u2d_.row(0).cwiseProduct(u2d_.row(0)) + u2d_.row(1).cwiseProduct(u2d_.row(1)));
    r2d.row(1) = (u2d_.row(0).cwiseProduct(u2d_.row(0)) + u2d_.row(1).cwiseProduct(u2d_.row(1)));
    ...
    left_errors.resize(u1d_.cols());
    right_errors.resize(u1d_.cols());
...
AutomaticEstimator::PairOfMatricesFandLambdas
AutomaticEstimator::solver_ku8pt(const AutomaticEstimator::G_polynomial &g1, const AutomaticEstimator::G_polynomial &g2,
                                 const AutomaticEstimator::G_polynomial &g3, const AutomaticEstimator::G_polynomial &g4,
                                 const AutomaticEstimator::G_polynomial &g5, const AutomaticEstimator::G_polynomial &g6,
                                 const AutomaticEstimator::G_polynomial &g7,
                                 const AutomaticEstimator::G_polynomial &g8) {
    ...
    AutomaticEstimator::PairOfMatricesFandLambdas res;
    for (size_t k = 0; k < sol.cols(); ++k) {
        if (std::abs(sol(0, k).imag()) < AutomaticEstimator::EPS) {
