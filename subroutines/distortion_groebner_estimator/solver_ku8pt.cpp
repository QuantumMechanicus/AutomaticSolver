...
namespace eight_points_problem {
    ...
    double
    AutomaticEstimator::estimate(Eigen::Matrix3d &F, double &Lambda, const std::string &lambdas_distribution_file,
                                 const std::string &inliers_output_file, int number_of_RANSAC_iterations,
                                 double lower_threshold, double upper_threshold) {
        std::size_t number_of_associated_points = helper_.getNumberOfPoints();
        ...
        std::fstream lmb_d(lambdas_distribution_file, std::fstream::app);
        Points u1d, u2d;
        u1d = helper_.getU1d();
        u2d = helper_.getU2d();
        for (std::size_t k = 0; k < number_of_RANSAC_iterations; ++k) {
            ...
            subset_Q2 << u2d.col(numbers[0]), u2d.col(numbers[1]), u2d.col(numbers[2]), u2d.col(
    ...
    double AutomaticEstimator::estimateQuantile(double hyp_lambda, const Eigen::Matrix3d &hyp_F) {
        helper_.setHypF(hyp_F);
        Eigen::Matrix<double, 1, 1> eigen_hyp_lambda;
        eigen_hyp_lambda(0, 0) = hyp_lambda;
        helper_.setHypLambdas(eigen_hyp_lambda);
        helper_.estimateQuantile();
        return helper_.getQuantile();
    }

    size_t AutomaticEstimator::findInliers(double hyp_lambda, const Eigen::Matrix3d &hyp_F,
