...
find $dir -name "points_*_left" | grep -o '[0-9]\+' | sed 's/^\(.*\)$/--inliers_f \1 --fund_f \1.f --lamd_f \1.l/g' | sort > numbers.txt
paste list.left list.right numbers.txt | sed 's#^\(.*\)$#./GroebnerAutomaticSolver \1#g' | parallel
