...
for n = 552:552
     ...
     n1 = strcat('./full_undistorted_images/', num2str(n),'/', 'C_', num2str(lambda,9), '_',num2str(lambda2,9), '.JPG');
     n2 = strcat('./full_undistorted_images/', num2str(n+1),'/', 'C_', num2str(lambda,9), '_',num2str(lambda2,9), '.JPG');
     ...
     myF =  dlmread(strcat('/home/danielbord/CLionProjects/AutomaticSolver/pipeline/', 'fundamental_matrices'));
     myF = myF(1:3, 1:3);
     myF = myF';
     ...
     figure;
     k =10;
     showMatchedFeatures(I1, I2, mp1(1:k,:),mp2(1:k,:),'montage','PlotOptions',{'ro','go','y--'});
     title('My undistorted points');
     epiLines = epipolarLine(myF, mp2(1:k,:));
